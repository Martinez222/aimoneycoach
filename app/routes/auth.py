...
async def register(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Email already registered" if locale == "en" else "Email deja inregistrat",
        )
...
async def login(
    ...
    if not user or not verify_password(data.password, user.password_hash):
        raise HTTPException(
            status_code=401,
            detail="Incorrect email or password" if locale == "en" else "Email sau parola incorecta",
        )
